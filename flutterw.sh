...
function collect_ios_product() {
    ...
    local basePath=$(cd `dirname $0`; pwd)
    local flutter_plugins="${basePath}/.flutter-plugins"
    local libs_dir="${basePath}/outProduct/libs" #产物ios输出目录
    local include_dir="${basePath}/outProduct/include" #产物ios输出目录
    local src_dir="${basePath}/outProduct/src"
    local resource_dir="${basePath}/outProduct/resource"

    rm -rf "${basePath}/outProduct"
    ...
    do
        echo "拷贝$cp_path 到 $libs_dir/${cp_path##*/}"
        cp -fr $cp_path $libs_dir/${cp_path##*/}
    done

    cp -fr $plugin_reg $include_dir/${plugin_reg##*/}

    local flutterPluginRegistrant_m="${include_dir}/FlutterPluginRegistrant/Classes/GeneratedPluginRegistrant.m"
